...
/**
 * Esta clase de acceso a datos de <strong>Estado</strong> se encarga
 ...
 * @version 1.0
 */
public class ManejadorDatosEstado {
     ...
     * @since 1.0
     */
    public String registrarEstado(Estado estado) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{call simuni_sp_registro_estado(?,?)}");
            cs.setString(1, estado.getNombreestado());
     ...
     * @since 1.0
     */
    public String modificarEstado(Estado estado) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{ call simuni_sp_actualizacion_estado(?,?,?)  }");
            cs.setString(1, estado.getNombreestado());
            cs.setInt(2, estado.getIdestado());
     ...
     * <strong>Estado</strong> de la base de datos..
     *
     ...
     * @since 1.0
     */
    public String eliminarEstado(Estado estado) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{ call simuni_sp_eliminacion_estado(?,?)  }");
            cs.setInt(1, estado.getIdestado());
            cs.registerOutParameter(2, java.sql.Types.VARCHAR);
            cs.execute();
            resp = cs.getString(2);
            Conexionmysql.cerrarConexion(con);
     ...
     * @since 1.0
     */
    public ResultSet listadoEstado(int desplazamiento, int paginacion) throws SQLException {
        ResultSet resp = null;

        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("SELECT *  FROM  simuni_vw_listado_estado  LIMIT " + desplazamiento + "," + paginacion + ";");
  ...
  /**
     ...
     * @since 1.0
     */
    public ResultSet listadoEstado() throws SQLException {
        ResultSet resp = null;

        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("SELECT *  FROM  simuni_vw_listado_estado;");
            resp = st.executeQuery();
     ...
     * @since 1.0
     */
    public Estado getEstado(int codigo) throws SQLException {
        Estado resp = null;
        Connection con = Conexionmysql.obtenerConexion();
        PreparedStatement st = con.prepareCall("{ call simuni_sp_obtener_estado(?)}", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        st.setInt(1, codigo);
        ResultSet rs = st.executeQuery();
        if (rs.next()) {
            resp = new Estado();
            resp.setIdestado(rs.getInt(1));
            resp.setNombreestado(rs.getString(2));
            rs.close();
        }
        return resp;
    }
    ...
    public int getCantidadFilas(String query) throws SQLException {
        int resp = 0;
        Connection con = Conexionmysql.obtenerConexion();
        PreparedStatement st = con.prepareCall(" {call simuni_sp_obtener_cantidad_reestado(?)}");
     ...
     * @since 1.0
     */
    public ResultSet busquedaEstado(String query, int desplazamiento, int paginacion) throws SQLException {
        ResultSet resp = null;
        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("{call simuni_sp_busqueda_estado(?,?,?)}");
            st.setString(1, query);
