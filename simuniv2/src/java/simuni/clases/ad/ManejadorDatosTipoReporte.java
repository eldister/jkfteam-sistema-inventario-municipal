...
/**
 * Esta clase de acceso a datos de <strong>Tipo de Reporte</strong> se encarga
 ...
 * @version 1.0
 */
public class ManejadorDatosTipoReporte {
     ...
     * @since 1.0
     */
    public String registrarTipoReporte(TipoReporte tiporeporte) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{call simuni_sp_registro_tiporeporte(?,?)}");
            cs.setString(1, tiporeporte.getNombretiporeporte());
     ...
     * @since 1.0
     */
    public String modificarTipoReporte(TipoReporte tiporeporte) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{ call simuni_sp_actualizacion_tiporeporte(?,?,?)  }");
            cs.setString(1, tiporeporte.getNombretiporeporte());
            cs.setInt(2, tiporeporte.getIdtiporeporte());
     ...
     * @since 1.0
     */
    public String eliminarTipoReporte(TipoReporte tiporeporte) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{ call simuni_sp_eliminacion_tiporeporte(?,?)  }");
            cs.setInt(1, tiporeporte.getIdtiporeporte());
            cs.registerOutParameter(2, java.sql.Types.VARCHAR);
            cs.execute();
            resp = cs.getString(2);
            Conexionmysql.cerrarConexion(con);
     ...
     * @since 1.0
     */
    public ResultSet listadoTipoReporte(int desplazamiento, int paginacion) throws SQLException {
        ResultSet resp = null;

        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("SELECT *  FROM  simuni_vw_listado_tiposreporte  LIMIT " + desplazamiento + "," + paginacion + ";");
     ...
     * @since 1.0
     */
    public TipoReporte getTipoReporte(int codigo) throws SQLException {
        TipoReporte resp = null;
        Connection con = Conexionmysql.obtenerConexion();
        PreparedStatement st = con.prepareCall("{ call simuni_sp_obtener_tiporeporte(?)}", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        st.setInt(1, codigo);
        ResultSet rs = st.executeQuery();
        if (rs.next()) {
            resp = new TipoReporte();
            resp.setIdtiporeporte(rs.getInt(1));
            resp.setNombretiporeporte(rs.getString(2));
            rs.close();
        }
        return resp;
    }
    ...
    public int getCantidadFilas(String query) throws SQLException {
        int resp = 0;
        Connection con = Conexionmysql.obtenerConexion();
        PreparedStatement st = con.prepareCall(" {call simuni_sp_obtener_cantidad_retiposreporte(?)}");
     ...
     * @since 1.0
     */
    public ResultSet busquedaTipoReporte(String query, int desplazamiento, int paginacion) throws SQLException {
        ResultSet resp = null;
        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("{call simuni_sp_busqueda_tiporeporte(?,?,?)}");
            st.setString(1, query);
