 ...
 * al <strong>Entrega de Documento</strong>.
 *
 * @author FchescO
 * @since 1.0
 * @version 1.0
 */
public class MostradorEntregaDocumento {
     ...
     *
     * @param rsentregadocumento ResultSet de Entrega de Documento
     * @param cantidadpaginas Para efectos de paginación, la cantidad de
     * páginas.
     * @return El texto a mostrar al usuario.
     * @since 1.0
     */
    public String Renderizar(ResultSet rsentregadocumento, int cantidadpaginas) {
        GrillaBase gril = new GrillaBase();
        gril.setUrlpaginacionlink(Recursos.Servers.MAINSERVER + "/proveedor/documentos?proceso=listado");
        ...
        gril.setOpcionverdocumentos(false);
        try {
            return gril.renderizar(rsentregadocumento, cantidadpaginas);
     ...
     * @since 1.0
     */
    public String RenderizarActualizacion(ResultSet rsentregadocumento, int cantidadpaginas) {
        GrillaBase gril = new GrillaBase();
        gril.setUrlpaginacionlink(Recursos.Servers.MAINSERVER + "/proveedor/documentos?proceso=listado");
        gril.setOpciommostrarfiltro(false);
        gril.setOpcionimprimir(false);
        ...
        gril.setOpcionverdocumentos(false);
        try {
            return gril.renderizarSoloCuerpoTabla(rsentregadocumento, cantidadpaginas);
        } catch (SQLException ex) {
            return "error " + ex.getMessage();
        }
    }
