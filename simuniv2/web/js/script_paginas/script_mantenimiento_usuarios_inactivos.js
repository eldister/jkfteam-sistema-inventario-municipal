...
function  sm_grilla_proceso_modificar(sm_grilla_item_id) {
    ...
    if (confirm("¿Realmente desea reactivar este usuario?")) {
        $.ajax({
            type: 'POST',
            url: SIMUNI_SERVER + "/usuario?proceso=reactivar&registro=" + sm_grilla_item_id,
            cache: false
        })
                .done(function (html) {
...
function sm_grilla_busqueda(sm_grillageneral_query) {
    sm_grilla_actualizargrillageneral("query_inactivos&query=" + sm_grillageneral_query);
}
