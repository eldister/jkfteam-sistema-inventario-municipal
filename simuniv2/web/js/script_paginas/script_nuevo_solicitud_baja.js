...
function setEventosBotonesFormulario() {
    ...
    $("#sm_btn_iractivos").click(function(e) {
        e.preventDefault();
        window.location.assign(SIMUNI_SERVER + '/solicitudbaja?proceso=listado');
    });
...
function iniciarCamposFechas() {
    if (!$("#txtfechabaja").val()) {
        setFechaDatePicker('txtfechabaja');
    }
}
...
function addEventoSubmitFormulario() {
    $("#formulario").submit(function(e) {
        ...
        element = $(this).children().find('#txtrazonbaja');
        isformulariovalido = (isformulariovalido && campoValido(element));
        ...
        if (isformulariovalido&&!Error_Existente) {
            if (confirm("Esta a punto de enviar una solicitud de baja un Activo, ¿Desea continuar?")) {
                $(this).unbind();
                $(this).submit();
            }
        }
    ...
    element.change(function() {
        $.ajax({
            ...
            if (html.length > 0) {
                ...
                setEstadoCampo(element, 1);
                Error_Existente = false;
                element.attr('title', 'El activo ingresado existe');                
                ...
                element.attr('title', 'El activo no se encuentra registrado');
            }
        }
