function addEventosSubmitFormulario() {
    $("#formulario_mantenimiento_reseteo_clave").submit(function (e) {
       ...
       var element = $(this).children().find('#txtpassword1');
        ...
        isformulariovalido = (isformulariovalido && campoValido(element));
        element = $(this).children().find('#cmbusuario');
        isformulariovalido = (isformulariovalido && campoValido(element));
        ...
        if (isformulariovalido) {
            if (confirm("Esta a punto resetear la clave del usuario, ¿Desea continuar?")) {
                $(this).unbind();
                $(this).submit();
            }
        }
...
function setEstadoCampo(element, value) {
    if (value == 3) {
        ...
        element.attr('title', 'Rellena / Corrige los errores de este campo');
