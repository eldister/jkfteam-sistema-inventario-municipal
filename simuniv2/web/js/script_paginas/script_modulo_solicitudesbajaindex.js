$(document).ready(function() {

    sm_grillageneral_inicializar();
    sm_grilla_irapagina_paginacioneventhandler();

    // alert(window.location);
    // inicializarpaginamantenimiento();
});


function sm_grilla_proceso_verdetalle(sm_grilla_item_id){
    
    var sm_form = $('<form  method="POST" action="' + SIMUNI_SERVER + '/solicitudbaja">');
    sm_form.appendTo('body');
    sm_form.append('<input type="hidden" value="' + sm_grilla_item_id + '" name="registro">');
    sm_form.append('<input type="hidden" value="solicitud_baja" name="proceso">');

    sm_form.submit();
}
function  sm_grilla_proceso_eliminar(sm_grilla_item_id) {
    var sm_form = $('<form  method="POST" action="' + SIMUNI_SERVER + '/solicitudbaja">');
    sm_form.appendTo('body');
    sm_form.append('<input type="hidden" value="' + sm_grilla_item_id + '" name="registro">');
    sm_form.append('<input type="hidden" value="eliminar_solicitud" name="proceso">');

    sm_form.submit();
}






function sm_grilla_busqueda(sm_grillageneral_query) {
    sm_grilla_actualizargrillageneral("query&mod=grilla&query=" + sm_grillageneral_query);
}


function sm_grilla_actualizargrillageneral(proceso) {
    $.ajax({
        type: 'POST',
        url: SIMUNI_SERVER + "/solicitudbaja?proceso=" + proceso,
        contentType: ' charset=utf-8',
        success: function(data) {
            console.info("correcto" + data);
            $("#sm_div_puratablecontainertarget").html(data);
            sm_grillageneral_refrescareventosgrilla();

            sm_grilla_irapagina_paginacioneventhandler();
            //actualizar grilla
        },
        beforeSend: function() {
            $("#sm_div_puratablecontainertarget").addClass('sm_div_grillageneralloading');
            // $('#sm_respuesta').css('font-size','3em');
        },
        complete: function() {
            // $('#sm_respuesta').css('font-size','0.51em');
            $("#sm_div_puratablecontainertarget").removeClass('sm_div_grillageneralloading');
        },
        error: function(data) {
            console.info(data);

        }

    });


}

function sm_grilla_irapagina_paginacioneventhandler() {
    $(".sm_div_elementlinkpag").click(function(e) {
        // alert('entree')
        var link = $(this).attr('href');
        var searchvalue = $(this).parents('#sm_grilla_maincontainer').children().find('#sm_div_txtcriteriobusquedagrilla').val();
        if (searchvalue) {
            //    alert('entre4e')
            link += '&query=' + searchvalue;
        }
        link += sm_grillageneral_mostrarinbajas;
        $(this).attr('href', link);
    });
}

function iniciarValores(i) {

    $("#sm_checkgrillageneral_mostrarinbajas").prop('checked', i);
    if ($("#sm_checkgrillageneral_mostrarinbajas").is(":checked")) {

        sm_grillageneral_mostrarinbajas = "&mostrar_inbajas=si";
        sm_grillageneral_buscarinbajas = true;

    }
}

function sm_grilla_proceso_mostrarinbajas(i) {
    var cururl = window.location.toString();

    if (i) {
        if (cururl.indexOf(sm_grillageneral_mostrarinbajas) < 0) {
            cururl += sm_grillageneral_mostrarinbajas
        }
    } else {
        console.info("estoy eliminando " + sm_grillageneral_mostrarinbajas)
        cururl = cururl.replace(sm_grillageneral_mostrarinbajas, "");

    }
    window.location.assign(cururl);

}
