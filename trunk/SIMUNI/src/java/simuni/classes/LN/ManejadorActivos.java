...
public class ManejadorActivos {
     ...
     * @return verdadero en caso de que se halla ingresado correctamente o false
     * si no lo fue
    ...
    public boolean agregarActivoArticulo(Activos_Articulos to_articulo) throws Exception {
        try {
            ...
            if (to_articulo.getPa_identificadorActivo() == null) {
                return false;
            }
            //porcentaje
            if (to_articulo.getPb_porcentajeDepreciacion() < 0 && to_articulo.getPb_porcentajeDepreciacion() > 100) {
                return false;
            }
            if (to_articulo.getPb_porcentajeRescate() < 0 && to_articulo.getPb_porcentajeRescate() > 100) {
                return false;
            }
    ...
    /**
     * Este método es para la actualización de la información de los activos
     * articulos almacenados en la base de datos
     *
     * @param to_articulo el artículo al cual se le va a modificar los datos
     * @return true en caso de que el articulo halla sido actualizado con exito
    ...
    public boolean modificarActivoArticulo(Activos_Articulos to_articulo) {
        ...
        try {
            ManejadorDatosActivos manejadordatosactivos = new ManejadorDatosActivos();
            resultado = manejadordatosactivos.modificarActivoArticulo(to_articulo);
            UtilidadesServlet.registrarProcesoSistema("modificarActivoArticulo", "Se modificó con éxito el activo");
        } catch (Exception ex) {
            UtilidadesServlet.registrarErrorSistema("modificarActivoArticulo", ex.getMessage());
            ex.printStackTrace();
            return false;
        }
        return resultado;
    ...
    /**
     * Este método desactiva un registro en la base de datos especificado por el
     * código del activo
     *
     * @param ta_codigoactivo código de identificación del artículo activo
     * @return true cuando el artículo activo ha sido desactivado con éxito o
    ...
    public boolean desactivarActivoArticulo(String ta_codigoactivo) {
        ...
        try {
            ...
            ManejadorDatosActivos manejadordatosactivos = new ManejadorDatosActivos();
            resultado = manejadordatosactivos.desactivarActivoArticulo(ta_codigoactivo);
    ...
    /**
     * Este método es utilizado para obtener una lista completa de los artículos
     * activos registrados en la base de datos
     *
     * @param desplazamiento
     * @param paginacion Los dos anteriores parametros son limitar el total de
     * resultados obtenidos por la consulta a la base de datos
    ...
    /**
     * Este método obtiene el total registros de los artículos activos
    ...
    /**
     * Este método obtiene un activo articulo en especifico definido por el
     * identificador del activo en otras palabras es una busqueda de un activo
     * articulo en particular utilizando su codigo
     *
     * @param ta_codigoactivo es el codigo del activo articulo
     * @return el articulo especificado o un nulo en caso de que no se este
     * registrado
     * @since 1.0
     */
    ...
    /**
     * Este método obtiene una lista de las fotografías o imagenes de todos los
     * activos registrados en la base de datos
     *
     * @param tn_codigoactivo codigo del artículo
     * @return un ArrayList con todas las urls y nombres de las imagenes
     * contenidos en la base de datos retorna nulo en caso de que no halla
     * podido obtener las imagenes
     * @since 1.0
     */
    ...
    /**
     * Este método hace una validación para comprobar si un artículo activo
     * existe actualmente en la base de datos
     *
     * @param ta_codigoactivo código del artículo activo
     * @return verdadero en caso de que el activo exista o false en caso de que
     * no
     * @since 1.0
     */
    ...
    /**
     * Este método obtiene un numero entero positivo del total de los activos
    ...
    public int getNumeroActivosRegistrados() {
        ...
        try {
            ManejadorDatosActivos manejadordatosactivos = new ManejadorDatosActivos();
            resultado = manejadordatosactivos.getCantidadRegistrosActivosArticulos();
     ...
     *
     * @param query consulta de búsqueda para el LIKE en MySQL Los siguientes
     * parámetros son para obtener y limitar el total de resultados obtenidos
     * por la consulta
     * @param desplazamiento
     * @param paginacion
     * @return un ArrayList con todos los artículos activos encontrados,
     * devuelve nulo en caso de que no se halla encontrado nada
     * @since 1.0
     */
