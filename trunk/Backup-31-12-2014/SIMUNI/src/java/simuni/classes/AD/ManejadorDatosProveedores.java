...
public class ManejadorDatosProveedores {
    ...
    public boolean guardarProveedor(ProveedorFisico prove, Collection<Part> parts) {
        ...
        if (parts != null) {
            ...
            System.out.print(prove.getPa_correoElectronico());
            String ruta = Servidor.SSA.CARPETAPROVEEDORES.toString() + "proveedorprueba" + "\\" + sDate;
            while (part.hasNext()) {
                Part au = part.next();
                   String name = UtilidadesServlet.getFilename(au);
                   if (au != null&&name!=null) {
               
                    try {
                        manejadorarchivos.guardarArchivo(ruta, name, au.getInputStream());
                        //registrarlo en bd
