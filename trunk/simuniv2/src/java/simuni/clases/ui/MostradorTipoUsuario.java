 ...
 * @version 1.0
 */
public class MostradorTipoUsuario {
    /**
     ...
     *
     * @param rstiposusuario ResultSet de Tipo de Usuario
     * @param cantidadpaginas Para efectos de paginación, la cantidad de
     * páginas.
     * @return El texto a mostrar al usuario.
     * @since 1.0
     */
    public String Renderizar(ResultSet rstiposusuario, int cantidadpaginas) {
        GrillaBase gril = new GrillaBase();
        gril.setUrlpaginacionlink(Recursos.Servers.MAINSERVER + "/tipousuario?proceso=listado");
        ...
        try {
            return gril.renderizar(rstiposusuario, cantidadpaginas);
     ...
     * @since 1.0
     */
    public String RenderizarActualizacion(ResultSet rstiposusuario, int cantidadpaginas) {
        GrillaBase gril = new GrillaBase();
        gril.setUrlpaginacionlink(Recursos.Servers.MAINSERVER + "/tipousuario?proceso=listado");
        gril.setOpciommostrarfiltro(false);
        gril.setOpcionimprimir(false);
        gril.setOpcionver(false);
        try {
            return gril.renderizarSoloCuerpoTabla(rstiposusuario, cantidadpaginas);
        } catch (SQLException ex) {
            return "error " + ex.getMessage();
        }
    }
