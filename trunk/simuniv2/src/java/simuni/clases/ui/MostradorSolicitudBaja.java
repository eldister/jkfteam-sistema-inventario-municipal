 ...
 * al <strong>SolicitudBaja</strong>.
 *
 * @author FchescO
 * @since 1.0
 * @version 1.0
 */
public class MostradorSolicitudBaja {
     ...
     *
     * @param rssolicitudbaja ResultSet de SolicitudBaja
    ...
    public String Renderizar(ResultSet rssolicitudbaja, int cantidadpaginas) {
        ...
        gril.setUrlpaginacionlink(Recursos.Servers.MAINSERVER + "/solicitudbaja?proceso="+(isIsAdmin()?"listado_admin":"listado"));
        gril.setUrlagregaropcionagregar(Recursos.Servers.MAINSERVER + "/solicitudbaja?proceso=nuevo");
        gril.setOpcionagregar(!isAdmin);
        gril.setUsarAgregarself(false);
        gril.setOpciommostrarfiltro(false);
        gril.setOpcionimprimir(false);
        gril.setOpcionver(false);
        gril.setOpcionactualizar(!isAdmin);
