...
/**
 * Esta clase se encarga de preparar lo que se va a mostrar al llanta,
 ...
 * al <strong>Tipo de Usuario</strong>.
 *
 * @author FchescO
 * @since 1.0
 * @version 1.0
 */
public class MostradorTipoLlanta{
    /**
     ...
     *
     * @param rstiposllanta ResultSet de Tipo de Usuario
     * @param cantidadpaginas Para efectos de paginación, la cantidad de
     * páginas.
     * @return El texto a mostrar al llanta.
     * @since 1.0
     */
    public String Renderizar(ResultSet rstiposllanta, int cantidadpaginas) {
        GrillaBase gril = new GrillaBase();
        gril.setUrlpaginacionlink(Recursos.Servers.MAINSERVER + "/tipollanta?proceso=listado");
        ...
        try {
            return gril.renderizar(rstiposllanta, cantidadpaginas);
     ...
     * @since 1.0
     */
    public String RenderizarActualizacion(ResultSet rstiposllanta, int cantidadpaginas) {
        GrillaBase gril = new GrillaBase();
        gril.setUrlpaginacionlink(Recursos.Servers.MAINSERVER + "/tipollanta?proceso=listado");
        gril.setOpciommostrarfiltro(false);
        gril.setOpcionimprimir(false);
        gril.setOpcionver(false);
        try {
            return gril.renderizarSoloCuerpoTabla(rstiposllanta, cantidadpaginas);
        } catch (SQLException ex) {
            return "error " + ex.getMessage();
        }
    }
