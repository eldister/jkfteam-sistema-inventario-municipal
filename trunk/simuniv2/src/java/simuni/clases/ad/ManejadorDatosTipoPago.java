...
/**
 * Esta clase de acceso a datos de <strong>Tipo de Pago</strong> se encarga
...
public class ManejadorDatosTipoPago {
     ...
     * @since 1.0
     */
    public String registrarTipoPago(TipoPago tipopago) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{call simuni_sp_registro_tipopago(?,?)}");
     ...
     * @since 1.0
     */
    public String modificarTipoPago(TipoPago tipopago) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{ call simuni_sp_actualizacion_tipopago(?,?,?)  }");
            cs.setString(1, tipopago.getNombretipopago());
            cs.setInt(2, tipopago.getIdtipopago());
     ...
     * @since 1.0
     */
    public String eliminarTipoPago(TipoPago tipopago) throws SQLException {
        String resp = "";
        try {
            Connection con = Conexionmysql.obtenerConexion();
            CallableStatement cs = con.prepareCall("{ call simuni_sp_eliminacion_tipopago(?,?)  }");
            cs.setInt(1, tipopago.getIdtipopago());
            cs.registerOutParameter(2, java.sql.Types.VARCHAR);
            cs.execute();
            resp = cs.getString(2);
            Conexionmysql.cerrarConexion(con);
     ...
     * @since 1.0
     */
    public ResultSet listadoTipoPago(int desplazamiento, int paginacion) throws SQLException {
        ResultSet resp = null;

        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("SELECT *  FROM  simuni_vw_listado_tipospago  LIMIT " + desplazamiento + "," + paginacion + ";");
     ...
     * @since 1.0
     */
    public ResultSet listadoTipoPago() throws SQLException {
        ResultSet resp = null;

        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("SELECT *  FROM  simuni_vw_listado_tipospago;");
            resp = st.executeQuery();
     ...
     * @since 1.0
     */
    public TipoPago getTipoPago(int codigo) throws SQLException {
        TipoPago resp = null;
        Connection con = Conexionmysql.obtenerConexion();
        PreparedStatement st = con.prepareCall("{ call simuni_sp_obtener_tipopago(?)}", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        st.setInt(1, codigo);
        ResultSet rs = st.executeQuery();
        if (rs.next()) {
            resp = new TipoPago();
            resp.setIdtipopago(rs.getInt(1));
            resp.setNombretipopago(rs.getString(2));
            rs.close();
        }
        return resp;
    }
    ...
    public int getCantidadFilas(String query) throws SQLException {
        int resp = 0;
        Connection con = Conexionmysql.obtenerConexion();
        PreparedStatement st = con.prepareCall(" {call simuni_sp_obtener_cantidad_retipospago(?)}");
     ...
     * @since 1.0
     */
    public ResultSet busquedaTipoPago(String query, int desplazamiento, int paginacion) throws SQLException {
        ResultSet resp = null;
        try {
            Connection con = Conexionmysql.obtenerConexion();
            PreparedStatement st = con.prepareCall("{call simuni_sp_busqueda_tipopago(?,?,?)}");
            st.setString(1, query);
