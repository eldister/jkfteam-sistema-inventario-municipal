...
<%
    ...
    try {
        tipopago = (TipoPago) request.getAttribute("nuevoregistro");
        respuesta = (Respuesta) request.getAttribute("respuesta");
        if (respuesta != null && tipopago != null) {
            proceso = true;
        }
...
<form class="sm_div_formulario" method="POST" id="formulario_registro">
    <fieldset id="sm_fs_articulos">
        <legend style="<%out.print((error && proceso) ? "color:red;" : "");%>" >Registro de Tipo Pago</legend>
        <div class="sm_form_registroinformacion">
            <table class="sm_tb_campos table">
                <tr class="sm_table_formulariofila">
                    <th>
                        <label>Nombre de Tipo de Pago</label>
                    </th>
                    <td>
                        <input type="text" value="<%out.print((error && proceso && tipopago != null) ? tipopago.getNombretipopago(): "");%>" class="form-control" id="txtnombretipopago" required="required"  name="txtnombretipopago">
                    </td>  
                </tr>  
                <tr class="sm_table_formulariofila">
                    <td colspan="2">
