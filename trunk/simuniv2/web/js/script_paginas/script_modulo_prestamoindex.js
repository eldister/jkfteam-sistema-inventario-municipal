...
function  sm_grilla_proceso_modificar(sm_grilla_item_id) {
    ...
    var sm_form = $('<form  method="get" action="' + SIMUNI_SERVER + '/prestamo">');
...
//    var sm_form = $('<form  method="get" action="' + SIMUNI_SERVER + '/prestamo/documentos">');
//    sm_form.appendTo('body');
//    sm_form.append('<input type="hidden" value="' + sm_grilla_item_id + '" name="registro">');
...
function sm_grilla_proceso_agregarnuevo() {
    $.ajax({
        url: SIMUNI_SERVER + "/prestamo?proceso=nuevo",
        cache: false
    })
            .done(function(html) {
...
function sm_grilla_actualizargrillageneral(proceso) {
    $.ajax({
        type: 'POST',
        url: SIMUNI_SERVER + "/prestamo?proceso=" + proceso,
